...
local function isTarget(model)
    ...
    local screenName = textLabel and textLabel.Text or "" [cite: 1, 2]
...
task.spawn(function()
    ...
    while true do
        local char = player.Character [cite: 2, 3]
...
-- [[ AUTO PURCHASE DENGAN PERBAIKAN ERROR ]]
ProximityPromptService.PromptShown:Connect(function(prompt)
    ...
    if model and isTarget(model) then [cite: 5]
        ...
        -- Cek beberapa variasi fungsi fireproximityprompt agar tidak error nil
        if fireproximityprompt then
            fireproximityprompt(prompt)
        elseif fireprompt then
            fireprompt(prompt)
        else
            -- Jika executor tidak mendukung, gunakan cara alternatif (klik manual via VirtualUser)
...
task.spawn(function()
    while true do
        ...
        if char and backpack then [cite: 6]
...
print("KAMIAPA: Fix Nil Error & Anti-AFK Aktif!") [cite: 10]
